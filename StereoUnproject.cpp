...
// Frank Luna's HR Macro to clean up the error handling and make 
// everything much more clear. This will bring up a dialog box with
// the error and call.  Let's use it always, even release builds.
...
bool CreateWindowAndDevice()
{
	...
	// Setup the effect
	{
		...
		HR(g_D3D9Device->GetBackBuffer(0, 0, D3DBACKBUFFER_TYPE_MONO, &backBuffer));
		{
			HR(backBuffer->GetDesc(&bufferDesc));
			HR(g_D3D9Device->CreateTexture(bufferDesc.Width * 2, bufferDesc.Height, 0, D3DUSAGE_RENDERTARGET, 
				bufferDesc.Format, D3DPOOL_DEFAULT,
				&g_StereoTexture, nullptr));
			HR(g_StereoTexture->GetSurfaceLevel(0, &g_TexSurface));
		}
		backBuffer->Release();
	}
